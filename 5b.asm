...
_prints:
  ...
  .checkinner: ; inner loop for the number we are checking to
    ...
    cmp byte [r14+r12], 1 ; check if a rule against our order exists ( so if order[second*100+first] == 1)
    je .wrong
  ...
  .wrong:; if order is wrong, we inverse the two numbers and start again (bubble sort like)
    inc r13
    mov [r8], r11
    mov [r9], r10
    mov r8, rbp
  ...
  .good:
    cmp r13, 0
    je .toogood ; if it's good without change then it's too good so we don't count
    mov rax, rbp
    sub rax, rsp
    shr rax, 4
